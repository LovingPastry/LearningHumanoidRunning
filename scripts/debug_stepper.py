...
# Allow running this script directly without `PYTHONPATH=.:$PYTHONPATH`
...
def run(env, policy, paused=False):
...
def main():
    ...
    parser.add_argument(
        "--paused",
        action="store_true",
        help="start viewer in paused mode",
    )
    ...
    run(env, policy, paused=args.paused)
    print("-----------------------------------------")
